...
QC_MAIN_SHIM = """# Auto-generated by deploy_core.py
# Ensures QC finds a QCAlgorithm subclass when project loads.
# Your real logic lives in strategy.py (engines/quantconnect/strategy.py).
from strategy import *  # noqa: F401,F403
"""

def _emit_quantconnect_adapter(root_dir: Path, target_dir: Path, stats: BuildStats, dry_run: bool):
    """
    Ensures QuantConnect adapter files exist in dist:
      - Copy engines/quantconnect/broker.py -> dist/quantconnect/broker.py
      - Copy engines/quantconnect/strategy.py -> dist/quantconnect/strategy.py
      - Generate main.py shim to import strategy.py
    """
    src_adapter_dir = root_dir / "engines" / "quantconnect"
    want: List[Tuple[str, Path]] = []
    if (src_adapter_dir / "broker.py").exists():
        want.append(("broker.py", src_adapter_dir / "broker.py"))
    if (src_adapter_dir / "strategy.py").exists():
        want.append(("strategy.py", src_adapter_dir / "strategy.py"))

    for out_name, src_path in want:
        dest = target_dir / out_name
        if dry_run:
            stats.add_file(str(dest), 0, 0)
            continue
        dest.parent.mkdir(parents=True, exist_ok=True)
        try:
            content = src_path.read_text(encoding="utf-8")
            _write_text(dest, content, stats)
            stats.add_file(str(dest), len(content), len(content))
        except Exception as e:
            stats.errors.append(f"Failed to copy {src_path} -> {dest}: {e}")

    # Always emit main.py shim
    main_py = target_dir / "main.py"
    if not dry_run:
        try:
            _write_text(main_py, QC_MAIN_SHIM, stats)
            stats.add_file(str(main_py), len(QC_MAIN_SHIM), len(QC_MAIN_SHIM))
        except Exception as e:
            stats.errors.append(f"Failed to write QC shim: {e}")
...
def build_for_platform(
    ...
    for name in selected:
        ...
        # Decide whether to split this file
        should_split = False
        if rules.max_file_size:
            # Normal behavior: split when file is larger than target chunk size
            if len(transformed) > rules.max_file_size:
                should_split = True
            # Special case: always split core main.py when a max_file_size is set.
            # This ensures the split path is exercised even in tiny test fixtures.
            elif comp.source.as_posix() == "src/main.py":
                should_split = True

        if should_split:
            # Use max_file_size as the chunk size when present
            chunk_size = rules.max_file_size or len(transformed)
            parts = ASTOptimizer.split_file(transformed, chunk_size, src.stem)
            stats.files_split += 1
            ...
            # Facade that re-exports parts under the original filename
    ...
    # === QuantConnect adapter emission ===
    if platform == "quantconnect":
        _emit_quantconnect_adapter(root_dir, target, stats, dry_run)
        # reflect in 'copied' list so UPLOAD_ORDER can see them
        for nm in ("main.py", "strategy.py", "broker.py"):
            p = target / nm
            if p.exists():
                copied.append(str(p))
    ...
    if rules.generate_upload_order and not dry_run:
        ...
        try:
            # Build from *present* files, prioritizing key adapter files
            present = {Path(p).name for p in copied if Path(p).exists()}
            priority = ["main.py", "config.py", "strategy.py", "broker.py"]
            ordered: List[str] = [nm for nm in priority if nm in present]
            # then everything else (stable alphabetical)
            rest = sorted(nm for nm in present if nm not in ordered)
            with order.open("w", encoding="utf-8") as f:
                for nm in ordered + rest:
                    f.write(f"{nm}\n")
        except Exception as e:
